...
where price = (select min(price) from assignment.products);
...
from assignment.sales s 
full join assignment.Products p
on s.product_id = p.product_id;
...
select c.first_name,c.last_name,p.product_name,s.quantity_sold,s.total_amount
from assignment.customers c
...
select concat(c1.first_name,c1.last_name) as first_customer,concat(c2.first_name,c2.last_name) as second_name,c1.membership_status 
from assignment.customers c1
join assignment.customers c2
on c1.membership_status = c2.membership_status and c1.customer_id <> c2.customer_id 
...
select p.product_name,SUM(s.quantity_sold)as sales
from assignment.sales s 
left join assignment.products p
on s.product_id = p.product_id 
...
select concat(c.first_name,' ',c.last_name) as cust_name,p.category
from assignment.customers c
left join assignment.sales s 
on c.customer_id = s.customer_id 
left join assignment.products p
...
select p.product_name ,SUM(s.quantity_sold) as total_qtys
from assignment.sales s
left join assignment.products p 
on s.product_id = p.product_id 
group by p.product_name;
...
select * from assignment.sales;
select concat(c.first_name,' ',c.last_name) as cust_name,s.sale_date
from assignment.sales s
left join assignment.customers c
on s.customer_id = c.customer_id
where s.sale_date>'2022-12-31'and s.sale_date<'2024-1-1';

-- 24. Write a query to find the customers with the highest total sales in 2023.
select concat(c.first_name,' ',c.last_name) as cust_name,s.total_amount,s.sale_date
from assignment.sales s
left join assignment.customers c
on s.customer_id = c.customer_id
where s.sale_date>'2022-12-31'and s.sale_date<'2024-1-1'
order by s.total_amount desc 
limit 1;--depends on if you want top 1,2,3 etc


-- 25. Write a query to join the `Products` and `Sales` tables and select the most expensive product sold.
select p.product_name ,s.quantity_sold,p.price,s.total_amount 
from assignment.sales s
left join assignment.products p 
on s.product_id = p.product_id 
...
(
select s.customer_id,SUM(s.total_amount) as total_sales_amount
from assignment.sales s 
...
select * from assignment.sales;
select concat(c.first_name,' ',c.last_name), sum(s.quantity_sold ) as total_qty
from assignment.sales s 
...
select p.product_name,AVG(s.quantity_sold) as avq_quatity
from assignment.sales s 
inner join assignment.products p 
on s.product_id=p.product_id 
group by p.product_name;
...
select sum(s.quantity_sold) as dec_2023_sales from 
assignment.sales s 
where s.sale_date >'2023-11-30' and s.sale_date <'2024-1-1'

-- 32. Write a query to find the total amount spent by each customer in 2023 and list the customers in descending order.
select concat(c.first_name,' ',c.last_name),SUM(s.total_amount) as total_amount_spent
from assignment.sales s
inner join assignment.customers c 
on s.customer_id =c.customer_id 
group by c.first_name,c.last_name,s.sale_date
having s.sale_date >'2022-12-31' and s.sale_date <'2024-1-1'
order by c.first_name desc;

-- 33. Write a query to find all products that have been sold but have less than 5 units left in stock.
select p.product_name,SUM(s.quantity_sold) as total_sales,p.stock_quantity 
from assignment.sales s 
inner join assignment.products p 
on s.product_id =p.product_id 
...
select concat(c.first_name,' ',c.last_name) as full_name,c.registration_date,s.sale_date,count(*) as count_of_sales
from assignment.sales s 
inner join assignment.customers c 
on s.customer_id = c.customer_id 
group by c.first_name,c.last_name,c.registration_date ,s.sale_date 
having   s.sale_date<= c.registration_date +7 ;
...
select concat(c.first_name,' ',c.last_name) as full_name,s.quantity_sold
from assignment.sales s 
inner join assignment.customers c 
on s.customer_id =c.customer_id 
...
select max(p.stock_quantity) as highest_stock, min(p.stock_quantity) as lowest_stock
from assignment.products p
...
select p.product_name,EXTRACT(YEAR FROM s.sale_date)as sales_year,EXTRACT(MONTH FROM s.sale_date)as sales_month,sum(s.quantity_sold) as total_sales
from assignment.sales s 
inner join assignment.products p 
on s.product_id =p.product_id 
...
select count(*)
from (
select p.product_name,EXTRACT(YEAR FROM s.sale_date) as sales_year,sum(s.quantity_sold)
from assignment.sales s
left join assignment.products p 
on s.product_id=p.product_id
where EXTRACT(YEAR FROM s.sale_date)=2023 
group by p.product_name,EXTRACT(YEAR FROM s.sale_date)
) as myt;
...
select concat(c.first_name,' ',c.last_name) as full_name,EXTRACT(YEAR FROM s.sale_date) as sales_year,sum(s.total_amount) as money_spent
from assignment.sales s
left join assignment.customers c
on s.customer_id = c.customer_id
group by c.first_name,c.last_name,EXTRACT(YEAR FROM s.sale_date)
having EXTRACT(YEAR FROM s.sale_date)=2023
order by money_spent desc;

-- 50. Write a query to select the products that have been sold more than 100 times and have a price greater than 200.
select p.product_name,count(*) as times_sold,p.price
from assignment.sales s 
left join assignment.products p 
on s.product_id = p.product_id 
group by p.product_name ,p.price
having count(*) >100 and p.price > 200;
